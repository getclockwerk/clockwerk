...
export default async function push(args: string[]): Promise<void> {
  ...
  // Run a merge pass before pushing to finalize adjacent sessions
  ...
  if (cloudTokens.size === 0) {
    info("No cloud projects to push.");
    return;
  }
  ...
  for (const token of cloudTokens) {
    // Backfill: bump sync_version for sessions with descriptions/summaries
    // that haven't been synced yet, so the dirty query picks them up
    if (includeDescriptions) {
      db.run(
        `UPDATE sessions SET sync_version = sync_version + 1
         WHERE project_token = ?
           AND description IS NOT NULL
           AND description_synced = 0
    ...
    // Get dirty sessions (sync_version > synced_version), excluding pulled sessions
    ...
    try {
      ...
      if (!res.ok) {
        ...
        error(`Push failed for ${token}: HTTP ${res.status} ${body}`);
        continue;
      }
      ...
      if (result.accepted.length > 0) {
        const syncedFlags = [
          "synced_version = sync_version",
          ...(includeDescriptions ? ["description_synced = 1"] : []),
          ...(includeSummaries ? ["summary_synced = 1"] : []),
        ].join(", ");
        const updateStmt = db.prepare(`UPDATE sessions SET ${syncedFlags} WHERE id = ?`);
      ...
      error(`Push error for ${token}: ${err instanceof Error ? err.message : err}`);
    }
  }
  ...
  if (projectCount === 0) {
    ...
    const parts = [`Pushed ${totalPushed} session(s) across ${projectCount} project(s)`];
